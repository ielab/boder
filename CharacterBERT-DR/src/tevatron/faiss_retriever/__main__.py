...
def main():
    ...
    logger.info(f'Corpus size {len(look_up)}.')

    query_files = glob.glob(args.query_reps)
    logger.info(f'Pattern match found {len(query_files)} query files.')

    q_rep_0, q_lookup_0 = pickle_load(query_files[0])
    shards = chain([(q_rep_0, q_lookup_0)], map(pickle_load, query_files[1:]))
    if len(query_files) > 1:
        shards = tqdm(shards, desc='Loading query files', total=len(query_files))

    q_lookups = []
    q_reps = q_rep_0
    for q_rep, q_lookup in shards:
        q_reps = np.vstack((q_reps, q_rep))
        q_lookups += q_lookup
    logger.info(f'Query size {len(q_lookups)}.')

    # use faiss gpu
    logger.info(f'Loading faiss index to gpu.')
    co = faiss.GpuMultipleClonerOptions()
    co.shard = True
    co.useFloat16 = True
    retriever.index = faiss.index_cpu_to_all_gpus(retriever.index, co, ngpu=1)
    ...
    if args.save_text:
        write_ranking(psg_indices, all_scores, q_lookups, args.save_ranking_to)
    else:
