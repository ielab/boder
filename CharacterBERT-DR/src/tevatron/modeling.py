...
from transformers import AutoModel, PreTrainedModel, AutoModelForSequenceClassification
...
class DenseModel(nn.Module):
    ...
    def build(
        ...
        if model_args.teacher_model_name_or_path:
            teacher_model = AutoModelForSequenceClassification.from_pretrained(model_args.teacher_model_name_or_path,
                                                                               **hf_kwargs)
...
class DistilTypoDenseModelKD(DistilTypoDenseModel):
    def forward(
            self,
            query: Dict[str, Tensor] = None,
            typo_query: Dict[str, Tensor] = None,
            passage: Dict[str, Tensor] = None,
            ce_batch: Dict[str, Tensor] = None,
    ):
        q_hidden, q_reps = self.encode_query(query)
        typo_q_hidden, typo_q_reps = self.encode_query(typo_query)
        ...
        if self.training:
            if self.train_args.negatives_x_device:
                raise ValueError('negatives_x_device cannot be set')
            ...
            if self.teacher_model:
                ...
                ranker_out = self.teacher_model(**ce_batch, return_dict=True)
                logits = ranker_out.logits

                if self.train_args.temperature is not None:
                    assert self.train_args.temperature > 0
                    logits = logits / self.train_args.temperature

                reranker_scores = logits.view(
                    self.train_args.per_device_train_batch_size,
                    self.data_args.train_n_passages
                )

                n_negatives = self.data_args.train_n_passages
                indices = torch.tensor([list(range(i * n_negatives, i * n_negatives + n_negatives))
                                        for i in range(self.train_args.per_device_train_batch_size)],
                                       device=scores.device)

                kd_loss = self.KL(self.log_softmax(scores.gather(1, indices)),
                                  self.log_softmax(reranker_scores.detach()))

                typo_kd_loss = self.KL(self.log_softmax(typo_scores.gather(1, indices)),
                                       self.log_softmax(reranker_scores.detach()))
                loss = kd_loss + typo_kd_loss + (ce_loss + kl_loss) * self.train_args.alpha
            else:
                loss = ce_loss + kl_loss
...
class SimLMForInference(DenseModelForInference):
    @torch.no_grad()
    def encode_passage(self, psg):
        if psg is None:
            return None, None
        outputs: BaseModelOutput = self.lm_p(**psg, return_dict=True)
        passage_embedding = self.l2_normalize(outputs.last_hidden_state[:, 0, :])
        return None, passage_embedding

    @torch.no_grad()
    def encode_query(self, qry):
        if qry is None:
            return None, None
        outputs: BaseModelOutput = self.lm_q(**qry, return_dict=True)
        query_embedding = self.l2_normalize(outputs.last_hidden_state[:, 0, :])
